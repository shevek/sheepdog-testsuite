 ...
 */
@Root(name = "node-info")
public class NodeInfoCommand extends AbstractCommand {

    private static final Log LOG = LogFactory.getLog(NodeInfoCommand.class);
    ...
    public static void run(final ControllerContext context, List<? extends Sheep> sheeps) throws ControllerException, InterruptedException {
        ...
        for (final Sheep sheep : sheeps) {
            executor.submit("Getting node info", new ControllerExecutor.Task() {

                @Override
                public void run() throws Exception {
                    NodeInfoCommand.run(context, sheep);
                }
            });
        }
        executor.await();
    ...
    public static void run(ControllerContext context, Sheep sheep) throws ControllerException, InterruptedException {
        ExecOperator operator = new ExecOperator(5000, "${COLLIE}", "node", "info", "-p", String.valueOf(sheep.getConfig().getPort()));
